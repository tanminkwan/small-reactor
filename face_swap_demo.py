...
def main():
    ...
    # 임베딩 저장 (이미지 파일명으로 저장)
    embedding_filename = face_image_path.stem + ".json"  # face.json
    embedding_path = faces_dir / embedding_filename
    save_embedding(source_face.embedding, embedding_path)
    
    # 원본 이미지도 faces 디렉토리에 복사
    face_copy_path = faces_dir / face_image_path.name
    save_image(face_image, face_copy_path)
    
    logger.info(f"임베딩 저장 완료: {embedding_path}")
    logger.info(f"원본 이미지 복사 완료: {face_copy_path}")
    ...
    logger.info(f"얼굴 {len(target_faces)}개 탐지 완료 (좌→우, 위→아래 순으로 정렬됨)")
    for i, face in enumerate(target_faces):
        bbox = face.bbox
        x1, y1, x2, y2 = bbox
        center_x = (x1 + x2) / 2
        center_y = (y1 + y2) / 2
        logger.info(f"  얼굴 {i+1}: bbox={bbox}, 중심=({center_x:.1f}, {center_y:.1f}), det_score={face.det_score}")
    ...
    logger.info(f"  - {face_copy_path}: 원본 이미지 복사본")
